          ...
          <form onSubmit={handleSubmit} className="contents">
            ...
            <FlexRowGroup className="items-stretch">
              <LayoutGroup>
                <AnimatePresence>
                  {!loading && (
                    <m.div transition={{ width: { duration: 0.24 }, opacity: { duration: 0.12 } }} exit={{ width: 0, opacity: 0, filter: 'blur(0.125rem)' }}>
                ...
                <m.div className="grow">
                  <Button disabled={loading} type="submit" className="relative w-full" variant="primary" size="normal">
                    <m.span className="origin-top" transition={{ duration: 0.16 }} animate={{ opacity: loading ? 0 : 1, filter: loading ? 'blur(0.125rem)' : 'none', y: loading ? -2 : 0, scale: loading ? 0.75 : 1 }}>
                      Sign in
                    </m.span>
                    <m.span transition={{ delay: 0.08, duration: 0.16 }} className="absolute inset-0 m-auto size-fit origin-bottom" initial={{ opacity: 0 }} animate={{ opacity: loading ? 1 : 0, filter: loading ? 'none' : 'blur(0.125rem)', y: loading ? 0 : 2, scale: loading ? 1 : 0.75 }}>
